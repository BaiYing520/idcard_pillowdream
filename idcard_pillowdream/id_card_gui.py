...
from idcard_pillowdream import id_card_utils, name_utils, utils, loading_alert
...
class IDGen:
    ...
    def show_ui(self, root):
        self.root = root
        root.title('枕梦服务器成员身份证生成工具')
        # root.geometry('640x480')
        root.resizable(width=False, height=False)
        link = Label(root, text='获取帮助:', cursor='hand2', foreground="#FF0000")
        link.grid(row=0, column=0, sticky=tkinter.W, padx=3, pady=3, columnspan=3)
        link.bind("<Button-1>", utils.open_url)

        link = Label(root, text='https://github.com/PillowDream-Server-Management-Group', cursor='hand2', foreground="blue")
        link.grid(row=0, column=2, sticky=tkinter.W, padx=26, pady=3, columnspan=4)
        link.bind("<Button-1>", utils.open_url)

        Label(root, text='ID:').grid(row=1, column=0, sticky=tkinter.W, padx=3, pady=3)
        ...
        Label(root, text='权限:').grid(row=1, column=4, sticky=tkinter.W, padx=3, pady=3)
        self.eNation = Entry(root, width=8)
        self.eNation.grid(row=1, column=5, sticky=tkinter.W, padx=3, pady=3)
        Label(root, text='注册年:').grid(row=2, column=0, sticky=tkinter.W, padx=3, pady=3)
        ...
        Label(root, text='备注:').grid(row=3, column=0, sticky=tkinter.W, padx=3, pady=3)
